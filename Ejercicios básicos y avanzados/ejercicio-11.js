// Calcular promedio mezclado: Crea una función que reciba por parámetro un array y cuando es un valor number lo sume y de lo contrario cuente la longitud del string y lo sume.

// Es un poco locura, pero podremos ejercitar nuestra lógica con este ejercicio.

// Puedes usar este array para probar tu función:

const mixedElements = [6,1,"Marvel",1,"hamburguesa","10","Prometeo",8,"Hola mundo",]; //como "10" va entre comillas se toma como string no como number.


function averageWord(list) {

    let suma = 0

    for (let variable of list){

    if (typeof variable === "string"){
        suma += variable.length
    }

    else{
        suma += variable
    }
    }
    return suma;
    
};

console.log("El promedio mezclado da -> " + averageWord(mixedElements));